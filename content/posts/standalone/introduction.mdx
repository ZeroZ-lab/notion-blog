---
title: Meilisearch 完整指南：从入门到精通
description: 'Meilisearch 是一款用 Rust 编写的开源搜索引擎，以闪电般的速度、开箱即用的相关性、优秀的开发者体验和易用性而备受关注。本文提供从基础到高级的完整学习路径。'
date: '2025-05-04'
category: 技术分享
tags: ['Meilisearch', '搜索引擎', 'Rust']
published: true
cover: /images/posts/引言/cover.jpg
---
...
- **`pagination`**:
    - **作用:** 控制分页行为。
    - **深度:** 核心是 `maxTotalHits` (默认1000)。这个设置限制了**一次搜索请求能访问到的最大结果文档数**（无论 `limit` 设置多大）。目的是防止用户进行极深的分页（如请求第 1000 页），这类操作对搜索引擎性能消耗巨大（需要计算并排序大量结果才能找到深页数据）。需要根据业务场景合理设置。
...
- **过滤 (Filtering) 进阶:**
    ...
    - **复杂示例:** 查找2000年后发行的非 G 级（`rating != 'G'`）的 Action 或 Thriller 类型电影，且必须包含 `overview` 字段：
...
- **排名规则 (Ranking Rules) 深度解析:**
    ...
    2. **`typo`****:** 惩罚错字。基于编辑距离计算。错字越少，排名越高。可通过 `typoTolerance` 设置微调（见 3.1）。
...
- **自定义排名规则 (****`asc(field)`****,** **`desc(field)`****):**
    ...
    - **放置策略:**
        ...
        - 放在**后面** (e.g., `sort` 之前): 主要用于最终的 tie-breaking。
...
- **迭代调优:** 相关性调优**没有银弹**。必须基于对业务、数据和用户行为的理解，通过 A/B 测试、用户反馈、搜索日志分析等方式，不断调整 `searchableAttributes` 顺序、`rankingRules`（包括自定义规则）、`synonyms` 等设置，是一个持续优化的过程。
